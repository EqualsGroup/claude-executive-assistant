...
name: sync
description: >
  Sync memory files from external sources (Notion, Slack, GitHub) using parallel
  subagents. Use when the user says "sync", "refresh", "pull latest", or
  "update from sources".
---
...
Convert the cutoff timestamp to a Unix timestamp (seconds since epoch) for Slack API calls.
...
#### Subagent 1: ~~documents Sources (Notion)
...
#### Subagent 2: ~~messaging Channels (Slack)
- Load the Slack tools using ToolSearch (query: "slack read channel")
...
#### Subagent 3: ~~messaging DMs (Slack)
- Load the Slack tools using ToolSearch (query: "slack search private")
- Use cached Slack user IDs from `sync/sources.md` (Slack User ID Cache table) — do NOT look up IDs via `slack_search_users` unless someone is missing from the cache
- Use `slack_search_public_and_private` to search for recent DM conversations with each key contact:
  - For each person in the "Key DM contacts to monitor" list, search: `in:<@THEIR_SLACK_ID> after:YYYY-MM-DD`
  - Also run a broad catch-all: `from:<@YOUR_SLACK_ID> after:YYYY-MM-DD` and `to:me after:YYYY-MM-DD`
...
#### Subagent 4: GitHub Activity
- Only launch if GitHub is configured in `sync/sources.md`
...
2. Draft proposed changes grouped by file:
   ...
   - **my-work.md** — new action items, commitments discovered in Slack, PRs pending review, in-flight PRs authored
...
- When adding work items discovered in Slack, follow the my-work.md conventions (Now/Next/Later tiers)
