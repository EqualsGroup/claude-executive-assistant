...
name: setup
description: >
  Interactive setup wizard — initializes the secretary memory folder, fills in
...
2. If not set, detect the environment:
   ...
   - **Task session** (Cowork with no mount): Stop and tell the user:
     "Secretary needs a Project session with a folder selected. Please start
     a new Project and select your Secretary folder."
...
Initialize git:
```bash
cd "$SECRETARY_ROOT" && git init && git add -A && git commit -m "Initialize secretary memory"
```
   ...
   **Method C: Personal Access Token file (required in Cowork / fallback elsewhere)**
   ...
   - If no token exists, offer two paths:
     ...
     **Token creation steps:**
     ...
     3. **Token name:** Something like "Secretary - Cowork"
...
/secretary:sync now to populate your memory files from existing data?"
