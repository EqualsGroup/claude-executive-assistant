...
| `slack-activity` | Slack search for messages the user sent in the relevant time window |
| `github-activity` | `gh` CLI: commits pushed, PRs opened/merged/reviewed, reviews given. Use specific API calls: `gh api search/commits`, `gh api search/issues` with appropriate date filters |
| `github-open-prs` | `gh` CLI to list all open PRs authored by the user. **For each PR, also fetch review status** (`gh api repos/ORG/REPO/pulls/NUMBER/reviews`) and latest commit date (`gh api repos/ORG/REPO/pulls/NUMBER/commits --jq '.[-1].commit.committer.date'`). Flag stale reviews where commits were pushed after the last review (likely means feedback was addressed, awaiting re-review) |
| `calendar` | Outlook calendar search for the specified time window (e.g. today, past week) |
...
**PR status is not the full picture.** GitHub review status is a point-in-time snapshot. CHANGES_REQUESTED doesn't mean the author hasn't addressed the feedback. Cross-reference PR review status with `my-work.md` notes and Slack conversations for the real state. The narrative from memory always takes precedence over the raw API status.
