...
description: >
  ...
  cross-references, and optimizes the secretary system. Use when the user says
...
Claude Code stores sessions per-project in `~/.claude/projects/`, keyed by the directory where `claude` was invoked. Since the secretary plugin can be active in **any** project, search ALL recent session files across all projects:

**Only process sessions modified since last improve:**
```bash
LAST_IMPROVE=$(head -1 $SECRETARY_ROOT/CLAUDE.md | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}Z' || echo "")
if [ -z "$LAST_IMPROVE" ]; then
  # Default to 7 days ago (portable)
  find ~/.claude/projects/ -name "*.jsonl" -mtime -7 2>/dev/null | head -20
else
  # Use -newermt for portable timestamp comparison (works on GNU find and macOS 10.11+)
  find ~/.claude/projects/ -name "*.jsonl" -newermt "$LAST_IMPROVE" 2>/dev/null | head -20
fi
```

If `find -newermt` is not available (older macOS), fall back to `-mtime` with a day-granularity approximation:
```bash
# Calculate days since last improve using portable date arithmetic
LAST_IMPROVE=$(head -1 $SECRETARY_ROOT/CLAUDE.md | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}Z' || echo "")
if [ -z "$LAST_IMPROVE" ]; then
  DAYS_AGO=7
else
  # Extract date part and compute days since then
  IMPROVE_DATE=$(echo "$LAST_IMPROVE" | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}')
  # Portable epoch calculation: try GNU date, then BSD date
