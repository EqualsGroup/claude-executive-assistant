...
**Task:**
- **Dated-section drift (check this FIRST):** Count headings in each memory file that name a date or a run rather than a topic — `## Recent Changes (<date>, <run> sync)`, `## <Month> <Day> ... Sync`, `### <date> sync — no new developments`, `(added by ... sync)`. Report the count and the line range they occupy as a percentage of the file. This is the highest-frequency failure mode in this system: sync/inbox runs append instead of merging, and the drift compounds silently until memory files blow past the Read limit. If any file is >30% dated sections, treat remediation as the top finding — the canonical sections are almost certainly stale too, because content stopped being promoted into them.
  **Remediation shape:** promote every still-live fact from the dated region into its canonical section, then move the dated region to `archive/<file>-log-archive-YYYY-MM-DD.md` rather than deleting it (the root is not always a git repo — never assume deletions are recoverable). Then verify the rules in CLAUDE.md `### Record Keeping` items 7-8 are present, since their absence is what let it happen.
