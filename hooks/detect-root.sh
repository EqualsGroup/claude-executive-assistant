#!/usr/bin/env bash
# Detect the Secretary memory root across environments:
...
if [ -d "/sessions" ]; then
  ...
  if [ -z "$MOUNT_DIR" ]; then
    ...
    echo "Secretary requires a Cowork Project session with a folder selected."
    echo "Please create a new Project and select your Secretary folder."
    exit 0
  fi
else
...
if [ -f "$SECRETARY_ROOT/CLAUDE.md" ]; then
  ...
  echo "Secretary memory not initialized. Run /secretary:setup to get started."
else
  echo "Secretary folder exists at $SECRETARY_ROOT but CLAUDE.md is missing."
  echo "Run /secretary:setup to repair, or check that your files are intact."
fi
